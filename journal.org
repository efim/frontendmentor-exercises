...
*** TODO image is changing with the theme change,
and icon on the change theme button
and also tailwind classes, but i'll read about that later
*** TODO overall state with task model:
1. list
2. text
3. isComplete
*** TODO task creation needs update function
*** TODO tasks list component with some group functions (and reordering things)
and counter of how many are left
and "filter" buttons
*** TODO single task display
