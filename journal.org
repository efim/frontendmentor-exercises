...
** TODO component with form : input and submit button
gets setter for the state
** TODO component that draws the table for data, when empty shows the loading thingy and overlay i guess
connected to state, which is on top level
gets stream of state.
** TODO adding map to lover component, returning function to set map center and point with coords.
